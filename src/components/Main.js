...
import { fetchMasjids, fetchAccounts, fetchTransactions, postMasjid, postExpense, postAccount, postIncome } from '../redux/ActionCreators';
...
function Main() {
    ...
    const MasjidWithId = ({match}) => {
      return(
        <MasjidDetail 
        masjid={allMasjids.masjids.filter((masjid) => masjid.id === parseInt(match.params.id,10))[0]}
        transactionLoading={allTransactions.isLoading}
        transactionErrMess={allTransactions.errMess}
        expenses = {allTransactions.transactions.filter((transaction)=> transaction.masjidId === parseInt(match.params.id,10) && transaction.transType === "expense")}
    ...
    const MasjidWithAccountId = ({match}) => {
      return(
        ...
        transactionLoading={allTransactions.isLoading}
        transactionErrMess={allTransactions.errMess}
        expenses = {allTransactions.transactions.filter((transaction) => transaction.masjidId === parseInt(match.params.id,10) && transaction.accountId === parseInt(match.params.accountId,10) && transaction.transType === "expense")}
        accounts = {allAccounts.accounts}
        fetchTransactions={fetchTransactions}
        fetchAccounts={fetchAccounts}
        postExpense={postExpense}
    ...
    const AccountWithId = ({match}) => {
      return(
        <AccountDetails 
        account={allAccounts.accounts.filter((account) => account.id === parseInt(match.params.id,10))[0]}
        transactionLoading={allTransactions.isLoading}
        transactionErrMess={allTransactions.errMess}
        transaction={allTransactions.transactions.filter((transaction)=> transaction.accountId === parseInt(match.params.id,10))}
        />
      );
    };
    ...
    const AccountIncomeWithId = ({match}) => {
      return(
        <AccountIncome
         account={allAccounts.accounts.filter((account) => account.id === parseInt(match.params.id,10))[0]}
         incomes = {allTransactions.transactions.filter((transaction)=> transaction.accountId === parseInt(match.params.id,10) && transaction.transType === "income")}
    ...
    const MasjidExpenseWithId = ({match}) => {
      return(
        <MasjidExpense 
        account={allAccounts.accounts.filter((account) => account.id === parseInt(match.params.id,10))[0]}
        expenses = {allTransactions.transactions.filter((transaction)=> transaction.accountId === parseInt(match.params.id,10) && transaction.transType === "expense")}
        income = {allTransactions.transactions.reduce((acc, list) => {
          if(list.accountId === parseInt(match.params.id,10) && list.transType === "income") 
          acc+= list.amount;
          return acc;
